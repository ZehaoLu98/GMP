...
void CuptiProfilerHost::SetUp(std::string chipName, std::vector<uint8_t> &counterAvailibilityImage)
{
    m_chipName = chipName;
...
CUptiResult CuptiProfilerHost::CreateConfigImage(
...
CUptiResult CuptiProfilerHost::EvaluateCounterData(
    size_t rangeIndex,
    std::vector<const char *> metricsList,
...
CUptiResult CuptiProfilerHost::Initialize(std::vector<uint8_t> &counterAvailibilityImage)
{
...
RangeProfilerTarget::RangeProfilerTarget(CUcontext ctx, RangeProfilerConfig &config) : m_context(ctx),
                                                                                       isProfilingActive(0),
                                                                                       mConfig(config)
{
    m_context = ctx;
...
CUptiResult RangeProfilerTarget::SetConfig(
...
CUptiResult RangeProfilerTarget::GetChipName(size_t deviceIndex, std::string &chipName)
{
...
CUptiResult RangeProfilerTarget::GetCounterAvailabilityImage(CUcontext ctx, std::vector<uint8_t> &counterAvailabilityImage)
{
